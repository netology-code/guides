...
До сегодняшнего дня вы выполняли все задания в так называемой «песочнице» [Codepen.io](https://codepen.io/). Это был удобный способ писать код и делиться им с преподавателем, не разбираясь при этом с тонкостями публикации сайтов в интернете.

Вся работа над дипломом будет вестись локально на вашем компьютере. Для этого вам понадобится редактор кода.
...
Чтобы быстро создать основную разметку страницы воспользуйтесь, встроенным в VS Code плагином Emmet. Он позволяет не писать все теги руками.
...
Обратите внимание, что все пути до файлов при локальной работе должны быть **относительными**.

[Шпаргалка по работе с относительными путями](../relative-link/README.md)

### Предварительный просмотр

Во время работы над дипломом вам понадобится постоянно видеть результат.
...
Чтобы увидеть не белый лист, напишите в HTML-разметке заголовок первого уровня (`<h1>`) с любым текстом. А в _styles.css_ зададайте заголовку красный цвет текста и рамку.

После внесения изменений сохраните файл (<kbd>CTRL</kbd> + <kbd>S</kbd>) и обновите страницу в браузере.
...
Скорее всего, вам понадобится обновлять только _index.html_ и _styles.css_.
...
3. После загрузки файлов внизу страницы нажмите «**Commit changes**».

![](./images/refresh.gif)

### Публикация диплома

Чтобы преподаватель увидел готовый сайт, его нужно опубликовать. Для этого используйте встроенный в Github сервис Pages.

1. На главной странице репозитория нажмите на ссылку «**Settings**».
...
4. Через несколько минут обновите страницу. Вверху появится ссылка на сайт с вашим дипломом. Её и отправляйте на проверку.
